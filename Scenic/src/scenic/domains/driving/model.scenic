...
# def checkPedestrianDectectedFlag(observations, pedestrian_position, ego_position):
#     resized_frame = cv2.resize(observations, (640, 640))
#     img = np.array(resized_frame)
#     inference_request = InferenceRequest(
#                                             inputs = [
#                                                 NumpyCodec.encode_input("image", img)
#                                             ]
#                                         )
#     r = requests.post('http://0.0.0.0:8080/v2/models/yolo_models/infer', json=inference_request.dict())
...
#         # print('check pedestrian detected flag')
#         # return True
#     if r.json()['outputs'][0]['data'][0] =='false':
#         return False  
#     else:
#         # print('check pedestrian detected flag') 
#         return True
...
def checkCautionBehaviour(observations, pedestrian_position, ego_position):
    ...
    # conf = r.json()['outputs'][0]['data'][1]

    # if  conf=='nan':
    #     return False  
    # else:
    #     confidence_value = np.float(conf)
